...
class Trainer:
    ...
    def fit(self):
        ...
        train_acc_history = []
        val_acc_history = []
        initial_lr =  deepcopy(self.learning_rate)
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_indices in batches_indices:
                ...
                for param_name, param in self.model.params().items():
                    ...
                    # print(f'before {param.value[0]}\n')
                    param.value = optimizer.update(param.value, param.grad, self.learning_rate)
                    # print(f'after {param.value[0]}\n')

                batch_losses.append(loss)

            if np.not_equal(self.learning_rate_decay, 1.0):
                # TODO: Implement learning rate decay
                # print(initial_lr, self.learning_rate)
                # self.learning_rate = initial_lr * (1 / (1 + self.learning_rate_decay * epoch))
                self.learning_rate = initial_lr * np.power(self.learning_rate_decay, epoch)
            ...
            # print('end of train_accuarcy\n')

            val_accuracy = self.compute_accuracy(self.dataset.val_X,
                                                 self.dataset.val_y)

            print("Loss: %.20f, Train accuracy: %f, val accuracy: %f\n" %
                  (batch_losses[-1], train_accuracy, val_accuracy))

            if self.best_val_accuarcy < val_accuracy:
                self.best_val_accuarcy = val_accuracy
                self.best_classifier = deepcopy(self.model)
